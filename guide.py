...
# Enhanced NLP utilities for resume generation
class EnhancedResumeNLP:
    """Advanced NLP utilities for resume generation with AI-powered suggestions"""
    
    # Comprehensive action verbs categorized by skill type
    ACTION_VERBS = {
        'leadership': [
    ...
    def suggest_action_verb(text, category='technical'):
        ...
        all_verbs = [v for verbs in EnhancedResumeNLP.ACTION_VERBS.values() for v in verbs]
        if first_word.capitalize() in all_verbs:
            return None
        
        # Suggest a verb from the category
        return EnhancedResumeNLP.ACTION_VERBS.get(category, EnhancedResumeNLP.ACTION_VERBS['technical'])[0]
    ...
    def analyze_resume_strength(resume_data):
        ...
        if resume_data.get('skills'):
            num_skills = sum(len(s.get('items', '').split(',')) for s in resume_data['skills'])
            if num_skills >= 8:
                score += 15
            elif num_skills >= 4:
                score += 10
    ...
    if data.get('education'):
        ...
        for edu in data['education']:
            edu_entry = {
                ...
                'highlights': EnhancedResumeNLP.format_bullet_points(edu.get('highlights', ''))
            }
            if edu.get('gpa'):
    ...
    if data.get('experience'):
        ...
        for exp in data['experience']:
            exp_entry = {
                ...
                'highlights': EnhancedResumeNLP.format_bullet_points(exp.get('highlights', ''))
            }
    ...
    if data.get('projects'):
        ...
        for proj in data['projects']:
            proj_entry = {
                'name': proj['name'],
                'date': proj.get('date', ''),
                'highlights': EnhancedResumeNLP.format_bullet_points(proj.get('description', ''))
            }
            if proj.get('url'):
    ...
    if data.get('skills'):
        ...
        for skill_cat in data['skills']:
            cv_data['cv']['sections']['Skills'].append({
                'label': skill_cat['category'],
                'details': skill_cat['items']
            })
...
def main():
    ...
    st.markdown("Create professional resumes with AI-enhanced content and real-time suggestions")
            ...
            with st.expander(f"Experience #{i+1}", expanded=(i==0)):
                ...
                if st.session_state.show_ai_suggestions and highlights:
                    ...
                    if not re.search(r'\d+', highlights):
                        st.info("💡 Tip: Add numbers or metrics to quantify your impact")
    ...
    with tab4:
        ...
        with col_skill:
            ...
            if st.button("➕ Add Skill Category"):
                st.session_state.skill_categories.append(f"Category {len(st.session_state.skill_categories) + 1}")
    ...
    with tab5:
        ...
        num_custom = st.number_input("Number of custom sections", min_value=0, max_value=5, value=0)
            ...
            with st.expander(f"Custom Section #{i+1}"):
                ...
                if section_title and section_content:
                    custom_sections.append({
                        'title': section_title,
                        'content': EnhancedResumeNLP.format_bullet_points(section_content)
                    })
    ...
    with tab6:
        ...
        if st.button("🔍 Analyze Resume", type="primary"):
            ...
            with col2:
                ...
                # Count total skills
                if resume_data.get('skills'):
                    total_skills = sum(len(s.get('items', '').split(',')) for s in resume_data['skills'])
